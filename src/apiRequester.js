...
const budgetsURL = baseURL + '/api/v2/agency/012/budget_function/'
...
async function getAgencyBudgets(toptierCode){
    const response = await getPage(budgetsURL);
    const result = response.results.map(ele => {
        return new Funding(ele["name"], ele["obligated_amount"], ele.children.map(child =>{
            new Funding(child["name"], child["obligated_amount"], [])
        }))
    });
    return result;
}

module.exports = {getAgencyNames, getPage, Agency};